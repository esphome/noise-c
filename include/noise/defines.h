...
/* Hash through the platform's mbedTLS rather than the selected backend's own
   ...
   platform refuses once the state exists, whether from memory pressure, a
   driver fault or a bad state, yields a random digest for that hash, random
   rather than a constant so two peers refused alike cannot agree on one;
   the layers above then produce a wrong result rather than an error, so a
   handshake fails on the peer's MAC and noise_hashstate_pbkdf2 hands back
   a wrong key. */
